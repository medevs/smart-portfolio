...
import { Home, Briefcase, BookOpen, Github } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-md bg-white bg-opacity-50">
            ...
            <div className="ml-10 flex items-baseline space-x-4">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="relative px-3 py-2 rounded-md text-sm font-medium text-gray-900 hover:text-black transition-colors duration-300 group flex items-center"
                >
                  <link.icon className="w-5 h-5 mr-2" />
                  {link.name}
                  <motion.span
                    className="absolute bottom-0 left-0 w-full h-0.5 bg-gray-900 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"
                    layoutId="underline"
                  />
                </Link>
              ))}
